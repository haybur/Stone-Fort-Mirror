...
int cor20 = 257, cor21 = 267, cor22 = 276, cor23 = 286, cor24 = 294, cor25 = 302, cor26 = 310, cor27 = 319, cor28 = NUM_LEDS;
...
void fillStats() {
	...
	spectrumValue[14] = fft1024.read(258, 359) * 10005;
...
void beatDetection(bool autoSwitch) {
	...
	// 2 = beat detected
	for (int i = 0; i < numFFTBins; i++) {
		if (spectrumValue[i] > average[i] + 3 * stDev[i]) {
			beatStrength[i] = beatStrength[i] + 2;
			if (spectrumValue[i] > 100)
				beatStrength[i] = beatStrength[i] + 3;
		}
...
void beatTiming() {
	if (beatDetected[2] == 2) { // if beat has been detected
		if (beatTimer[2] < 100) // and it isn't 100 ms from the last one
			beatDetected[2] = 1; // but if it is, cancel the beat
		else {
			beatInterval[2] = beatTimer[2];  // a beat has been detected. Get the time since last beat.
			if ((beatIntervalOld[2] - beatInterval[2]) < 30 && (beatIntervalOld[2] - beatInterval[2]) > -30) { // If the time 
				constantBeatCounter[2]++;
			}
			else {
				constantBeatCounter[2] = 0;
			}

			//Serial.print(beatIntervalOld[2]);
			//Serial.print("\t");
			//Serial.print(beatInterval[2]);
			//Serial.print("\t");
			//Serial.print(constantBeatCounter[2]);
			//Serial.print("\t");
			//Serial.println(beatPeakOld[2]);
			beatTimer[2] = 0;
			beatIntervalOld[2] = beatInterval[2];
		}
	}
}
...
double radius2;
void circle_double() {   // This mode shows 2 circles expanding from the middle of the smallest squares
	...
	if (pingPong == false) {
		for (int i = cor0; i < cor14; i++) {
			if (pow((ledx[i] - 13), 2) + pow((ledy[i] + 13), 2) - pow(radius, 2) > -10 && pow((ledx[i] - 13), 2) + pow((ledy[i] + 13), 2) - pow(radius, 2) < 10) leds[i] = ColorFromPalette(currentPalette, 0, 255, currentBlending);
		}
	...
	if (pingPong == true) {
		for (int i = cor0; i < cor14; i++) {
			if (pow((ledx[i] + 13), 2) + pow((ledy[i] - 13), 2) - pow(radius2, 2) > -10 && pow((ledx[i] + 13), 2) + pow((ledy[i] - 13), 2) - pow(radius2, 2) < 10) leds[i] = ColorFromPalette(currentPalette, 200, 255, currentBlending);
		}
...
void sweepInitiate(bool random) {
	coordMode = random8(0, 8);
...
void coordinatesToBeat() {
	...
	if (beatDetected[2] == 2) {
		coordMode = random8(0, 8);
		var = 0;
		speed = map(stDev[2], 2, 5, 1, 3);
	}
	speed = constrain(speed - .2, .1, 4); // old is .01
	//var += speed;
	var++;

	// Brightness goes down as the sweep goes
	brightness = 255 - .1 * pow(var, 2);
	brightness = constrain(brightness, 0, 105) + 150 ;
	...
	if (eq == 1) {  // 1 = normal
		for (int i = 0; i < NUM_LEDS; i++) {
			...
			if (slope * ledx[i] + yint - ledy[i] < 1 && slope *ledx[i] + yint - ledy[i] > -1) leds[i] = CHSV(hue, 255, brightness);
		}
	}
	if (eq == 2) {  // 2 = rotated
		for (int i = 0; i < NUM_LEDS; i++) {
			if (slope * ledy[i] + yint - ledx[i] < 1 && slope *ledy[i] + yint - ledx[i] > -1) leds[i] = CHSV(hue, 255, brightness);
		}
	}
...
void coordinatesToBeatExperimental() {
	...
	if (sweepStatus) {
		//addSparkles(cor0, cor28, 12, 1, -50, hue, false);
		//addSparkles(cor0, cor28, 21, 1, 200, hue, true);
		for (int i = 0; i < 1; i++) {
...
int getRandomPixel(int lowThresh, int highThresh) {
	int aRandomPixel = random16(lowThresh, highThresh);
	return aRandomPixel;
}
